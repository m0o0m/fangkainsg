...
一、内容：该类是星星构造器，游戏中所有星星的实例化都在这里；在构造星星的时候给它设置了一些属性和方法，这些属性和方法在游戏消除和排列时要用到。

二、控件：
2、ccui.ImageView:图片组件。

三、实现技术分析：
本类通过对图片组件的扩张，为星星设置了一些游戏消除相关的属性和方法，这些属性和方法的作用将会在下面的游戏代码中做详细的讲解。
这里把星星单独写成一个类，而不是在用到的时候直接以一个图片组件的形式实例化，这样做的好处是建立游戏松散耦合，能够更方便的系统管理，这也符合实际开发应用思想。
