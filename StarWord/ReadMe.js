...
三、讲解思路：
由于本主题是一个完整的游戏项目，所以讲解是以类为单位逐步推进的，有些类里面穿插了后续章节的类也是正常的，这就需要将后续章节都学习了才能前后连贯起来；如果不喜欢这样，觉得这样耦合太过严密了，也不用担心；因为每一个类既是与其他类耦合起来的，同时跟本类也是相对独立的；只要将耦合其他类的接口注释掉就可以了；每一个类的功能都可以单独的测试，整套主题将按照游戏的流程来讲解。

四、游戏中有部分通用的自定义组件及颜色的设定等，单独写在了一个GameUtil.js的文件中；这样做的目的是方便统一管理，可以做到一对多的控制效果，为各个js文件调用提供统一接口，具体内容如下：
GameUtil类
/*
 * 通用颜色
 */
