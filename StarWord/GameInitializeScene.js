...
一、场景内容：该场景是运行游戏后第一个出现在玩家眼前的界面，其中主要包含图片、文本、按钮，以及相应的特效，特效包
括音效、动作、粒子。
...
2、动作实现：场景中将两种动作（moveTo,EaseElasticOut)结合起来使用，而不是单一的移动或弹出，这样会给玩家一种绚丽的
感觉；对动作的控制可以按个人的审美调整（如：时间，坐标）。
3、粒子特效：粒子特效的贴图实现，粒子系统的实例化，以及粒子系统的位置设定；
4、定时器：设置定时器（schedule(...)来控制粒子效果的实例化，在该场景每隔一段时间就会实例化一次粒子爆炸效果，玩家
视线内场景中的粒子不会完全消失；通过定时器不断的产出粒子，这样游戏看起来更加绚丽。
5、音效：该场景右上角的喇叭按钮用来控制整个游戏背景音乐的播放和停止(cc.audioEngine.playMusic(...))；然后就是按钮
点击的音效播放（cc.audioEngine.playEffect(...)以及粒子爆炸的音效。
